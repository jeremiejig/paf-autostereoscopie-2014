...
void selectColor(in int texID, out vec4 color){
	...
	if(texID == 2) {
		...
		if(texID == 1){ color = texture2D(tex0, gl_TexCoord[0].st);}
		if(texID == 0) {
		...
		XSecondViewPixel = generateSecondViewPixel(depth1, coef,1.0);
		position2 = vec2(XSecondViewPixel ,gl_TexCoord[0].t);
		ColPx = texture2D(tex1, position2);
		color = ColPx;
	}
	if(texID == 6) {
	
		XSecondViewPixel = generateSecondViewPixel(depth1,2.0*coef,1.0);
		position2 = vec2(XSecondViewPixel ,gl_TexCoord[0].t);
		ColPx = texture2D(tex1, position2);
		color = ColPx;
	}
		if(texID == 5){ color = texture2D(tex1, gl_TexCoord[0].st);}
		if(texID == 4) {
